rule hisat2_index: 
    output:
        files=multiext(f"{utils.hisat2_index_path(utils.fasta())}", 
                ".1.ht2",
                ".2.ht2",
                ".3.ht2",
                ".4.ht2",
    ...
    params:
        fa=utils.fasta(),
        path=utils.hisat2_index_path(utils.fasta())
    ...
    conda:
        "../envs/count.yaml"
    shell:
        "hisat2-build -p {threads} {params.fa} {params.path} 2> {log}"


rule count:
    input: 
        fq="results/trimmed/{sample}.fastq.gz",
        idx_files=multiext(f"{utils.hisat2_index_path(utils.fasta())}", 
                    ".1.ht2",
                    ".2.ht2",
                    ".3.ht2",
                    ".4.ht2",
    ...
    params:
        mm=config["mismatch"],
        idx=utils.hisat2_index_path(utils.fasta())
    threads: config["resources"]["count"]["cpu"]
    resources:
        runtime=config["resources"]["count"]["time"]
    log:
